...
LEFT JOIN awards as a ON a.winner_id = m.model_id WHERE m.year > 1963;
...
SELECT b.name, m.year, m.name FROM brands AS b
JOIN models AS m ON b.brand_id = m.brand_id ORDER BY b.name;
...
FROM brands
FULL OUTER JOIN models ON models.brand_id = brands.brand_id
WHERE models.brand_id IS NULL;
...
SELECT brand_id FROM models GROUP BY brand_id
HAVING COUNT(*)>5;
